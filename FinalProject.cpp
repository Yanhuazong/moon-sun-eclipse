...
static const std::string texture_sky = "sky.jpg";
GLuint textureSky_id = -1; //Texture map for Moon
...
static const std::string moonview_vs("moonview_vs.glsl");
static const std::string moonview_fs("moonview_fs.glsl");
...
GLuint moonview_shader_program = -1;
...
void draw_moonview(const glm::mat4& P, const glm::mat4& V, float angP, float angS, float angM)
{
	float angt = 23.5;
	glm::mat4 T = glm::translate(earthpos);
	glm::mat4 Me = glm::rotate(angP, glm::vec3(0.0f, 0.0f, 1.0f))*T*glm::scale(glm::vec3(0.12f*mesh_data.mScaleFactor))*glm::rotate(angS, glm::vec3(0.0f, 0.398f, 0.917f));
	...
	glUseProgram(moonview_shader_program);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, textureMoon_id);
	int tex_loc = glGetUniformLocation(moonview_shader_program, "tex");
	if (tex_loc != -1)
	{
		glUniform1i(tex_loc, 0); // we bound our texture to texture unit 0
	}
	...
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_CUBE_MAP, cubemap_id);
	int cube_loc = glGetUniformLocation(moonview_shader_program, "cubemap");
	if (cube_loc != -1)
	{
		glUniform1i(cube_loc, 1); // we bound our texture to texture unit 1
	}

	int PVM_loc = glGetUniformLocation(moonview_shader_program, "PVM");
	if (PVM_loc != -1)
	{
		glm::mat4 PVM = P*V*M;
		glUniformMatrix4fv(PVM_loc, 1, false, glm::value_ptr(PVM));
	...
	int M_loc = glGetUniformLocation(moonview_shader_program, "M");
	if (M_loc != -1)
	{
		glUniformMatrix4fv(M_loc, 1, false, glm::value_ptr(M));
	}
	int Me_loc = glGetUniformLocation(moonview_shader_program, "Me");
	if (Me_loc != -1)
	{
		glUniformMatrix4fv(Me_loc, 1, false, glm::value_ptr(Me));
	}

	int V_loc = glGetUniformLocation(moonview_shader_program, "V");
	if (V_loc != -1)
	{
		glUniformMatrix4fv(V_loc, 1, false, glm::value_ptr(V));
	}
...
void draw_quad(const glm::mat4& P, const glm::mat4& V)
{
   ...
   glActiveTexture(GL_TEXTURE0);
   glBindTexture(GL_TEXTURE_2D, textureSky_id);
   int tex_loc = glGetUniformLocation(quad_shader_program, "tex");
   if (tex_loc != -1)
   {
	   glUniform1i(tex_loc, 0); // we bound our texture to texture unit 0
   }
...
//	draw_earth(P4, V4, 0.0, 0.0, moveMoon2);
	draw_quad1(P4, V4);
	draw_moonview(P4, V4, 0.0, 0.0, moveMoon2);
...
void initOpenGl()
{
   ...
   moonview_shader_program = InitShader(moonview_vs.c_str(), moonview_fs.c_str());
